...
#' (see \emph{References} for the respective publications).
#'
...
#' 208Pb/204Pb ratio must also be provided for consistency of the input with the
#' other age model functions.
#'
...
Albarede_et_al_2012 <- function(ratio_206_204, ratio_207_204, ratio_208_204)
{
  ...
  Model_age_func <- function(x,y) {
    stats::uniroot(function(X, a, b) {
      ((b-bp)/(a-ap)) - ((1/137.88)*((exp(l235*t0)-exp(l235*X))/((exp(l238*t0)-exp(l238*X))))) -
        (((mup*(exp(l238*X)-1))/(137.88*(a-ap)))*(((exp(l235*t0)-exp(l235*X))/(exp(l238*t0)-exp(l238*X))) - 
